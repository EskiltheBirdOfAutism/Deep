...
public class destroyCube : MonoBehaviour
{
    ...
    private void Awake()
    {
        hideSystem = GetComponentInParent<HideUnusedBlocks>();
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if(collision.gameObject.GetComponent<Tool>() != null)
        {
            if (collision.gameObject.GetComponent<Tool>().tool == ToolType.Pickaxe && collision.gameObject.GetComponent<Tool>().isEquiped)
            {
                ...
                // Notify the hide system before destroying
                if (hideSystem != null)
                {
                    hideSystem.OnCubeDestroyed(myPosition);
                    print("rehide");
                }
            }
        }
    ...
    {
        var fractureComponent = obj.AddComponent<destroyCube>();
