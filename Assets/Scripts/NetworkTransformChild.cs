...
public class NetworkTransformChild : NetworkBehaviour
{
    ...
    [SerializeField] public GameObject[] target_object = new GameObject[120];
    ...
    private void Start()
    {
        ...
        for (int _i = 0; _i < target_object.Length; _i++)
        {
            if (target_object[_i] != null)
            {
                target_position[_i] = target_object[_i].transform.localPosition;
                target_rotation[_i] = target_object[_i].transform.localRotation;
            }
        }
    ...
    private void Update()
    {
        ...
        if (IsOwner)
        {
            ...
            Vector3[] _pos = new Vector3[target_object.Length];
            Quaternion[] _rot = new Quaternion[target_object.Length];
            for (int _i = 0; _i < target_object.Length; _i++)
            {
                _pos[_i] = target_object[_i].transform.position;
                _rot[_i] = target_object[_i].transform.rotation;
            }
        ...
        else
        {
            ...
            for (int _i = 0; _i < target_object.Length; _i++)
            {
                if (target_object[_i] != null)
                {
                    target_object[_i].transform.localPosition = Vector3.Lerp(target_object[_i].transform.localPosition, target_object[_i].transform.position, Time.fixedDeltaTime * 10f);
                    target_object[_i].transform.localRotation = Quaternion.Slerp(target_object[_i].transform.localRotation, target_object[_i].transform.rotation, Time.fixedDeltaTime * 10f);
                }
            }
        }
    }
    ...
    private void UpdateTransformClientRpc(Vector3[] _pos, Quaternion[] _rot)
    {
        ...
        for (int _i = 0; _i < target_object.Length; _i++)
        {
            if (!IsOwner && target_object[_i] != null)
            {
